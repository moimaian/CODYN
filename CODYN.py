...
class MainWindow(QMainWindow):   
    ...
    def init_tab_home(self):
        ...
        title1.setStyleSheet("font-size:18pt;font-weight:bold;")
        layout.addWidget(title1)

        title2 = QLabel("Current version limited to protein/multi-ligand systems")
        title2.setAlignment(Qt.AlignCenter)
        title2.setStyleSheet("font-size:12pt;font-style:italic;")
...
QPushButton:hover {
    background-color: #FFFFFF;   /* Cinza médio ao passar o mouse */
    color: #000000;
}
QPushButton:pressed {
...
""")
    ...
    def init_tab6(self):
        ...
        self.print_res = QLineEdit("Within 4")
        ...
        form_layout.addRow("Decomp_Print_RES:", self.print_res)
...
""")
        run_btn.clicked.connect(self.salvar_mmpbsa)
        button_layout.addWidget(run_btn, alignment=Qt.AlignCenter)
        main_layout.addLayout(button_layout)

        back_btn = QPushButton("SHOW FORM")
...
""")
        back_btn.clicked.connect(self.voltar_form_mmpbsa)
    ...
    def init_tab7(self):
        ...
        self.combo_run_folder.addItems(run_folders)
        ...
        """)
        btn_view.setStyleSheet("font-size:12pt;font-weight:bold;background-color:green;color:#333333;")
    ...
    def salvar_mmpbsa(self):
        print("==> salvar_mmpbsa chamado")
        n_threads = int(self.n_threads.text())

        # Obter subpastas em RUNS
        runs_dir = os.path.join(self.codyn_dir, "RUNS")
        run_folders = [
        ...
        for run_folder in run_folders:
            ...
            if xtc_files:
                ...
                if match:
                    ...
                    t_ns_md_list.append("1")  # Valor padrão se não encontrar
            else:
                t_ns_md_list.append("1")  # Valor padrão se não encontrar
...
#  print_res='{config_mmpbsa['print_res']}',
/
"""
        ...
        # for run_folder, t_ns_md in zip(run_folders, t_ns_md_list):
        print(f"==> Criando MMPBSAWorker para {run_folder} com t_ns_md={t_ns_md}")
...
if __name__ == "__main__":
    ...
    def start_mainwindow(status):
        splash.close()
        # Mensagem para download de pastas faltantes
        if not status['folders_ok']:
