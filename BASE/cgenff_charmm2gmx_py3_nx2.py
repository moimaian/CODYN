...
class atomgroup:
	"""
	Classe que contém as estruturas e funções para armazenar/processar dados de grupos de átomos.
	"""
	...
	def read_charmm_rtp(self, rtplines, atomtypes):
		...
		for line in rtplines:
			...
			if line.startswith("ATOM"):
				entry = re.split('\s+', line.lstrip())
				atm[self.natoms] = {
					'type': entry[2], 'resname': self.name, 'name': entry[1],
				...
				# NOVO: usa a API moderna do NetworkX
			...
			if line.startswith("LONE"):
				entry = re.split('\s+', line.lstrip())
				atm[self.nvsites] = {
					'vsite': entry[2], 'at1': entry[3], 'at2': entry[4],
			...
			if line.startswith("BOND") or line.startswith("DOUB"):
				...
				for bondi in range(numbonds):
					...
					for idx in range(self.natoms):
						if atm[idx]['name'] == entry[(bondi * 2) + 1]:
							i = idx
						if atm[idx]['name'] == entry[(bondi * 2) + 2]:
							j = idx
					if i is None or j is None:
						print("Error:atomgroup:read_charmm_rtp> Atomname not found in top")
					if ((not is_lp(atm[i]['name'])) and (not is_lp(atm[j]['name']))):
						self.G.add_edge(i, j, order='1')
						self.nbonds += 1

			if line.startswith("IMP"):
				entry = re.split('\s+', line.lstrip())
				numimpr = int((len(entry)-2)/4)
				for impi in range(numimpr):
					i, j, k, l = None, None, None, None
					for idx in range(self.natoms):
						if atm[idx]['name'] == entry[(impi*4)+1]:
							i = idx
						if atm[idx]['name'] == entry[(impi*4)+2]:
							j = idx
						if atm[idx]['name'] == entry[(impi*4)+3]:
							k = idx
						if atm[idx]['name'] == entry[(impi*4)+4]:
							l = idx
...
if sys.version_info < (3, 8):
	print("Por favor, use Python 3.8 ou superior.")
	exit()
