    ...
    def run(self):
        ...
        try:
            ...
            for line in process.stdout:
                self.log_cell_signal.emit(line.rstrip())
            process.wait()
...
class PreparoCell(QWidget):
    ...
    def start_preparation(self, nome_proteina: str, nome_ligante: str):
        try:
            ...
            result=subprocess.run(f"{gmx} editconf -f complex.gro -o newbox.gro -bt {box_type} -c -d 1.0", shell=True, cwd=self.run_folder, capture_output=True, text=True)
            if result.stdout:
                self.log(result.stdout.strip())
            if result.stderr:
                self.log("Erro: " + result.stderr.strip())
