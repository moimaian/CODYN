...
class RequirementsInstaller(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout_mr.addWidget(QLabel(
            "Selected items will be installed into the CODYN Python environment (~/.venv/CODYN):" # "or system-wide when required. This installer prefers pip into the venv; when not available, it uses apt (requires sudo)."            
        ))
        ...
        # Venv
        self.check_venv = QCheckBox("Create/repair CODYN Python environment (~/.venv/CODYN)")
        ...
        self.check_cuda = QCheckBox("Install CUDA Toolkit")
        layout_mr.addWidget(self.check_cuda)

        self.check_gromacs = QCheckBox("Install GROMACS 2024.5")
        layout_mr.addWidget(self.check_gromacs)

        self.check_openbabel = QCheckBox("Install OpenBabel")
        layout_mr.addWidget(self.check_openbabel)

        self.check_gmxmmpbsa = QCheckBox("Install gmx_MMPBSA")
        ...
        self.btn_select_all = QPushButton("Select/Unselect All"); self.btn_select_all.clicked.connect(self.select_unselect_all)
    ...
    def select_unselect_all(self):
        checked = self.check_venv.isChecked()
        self.check_venv.setChecked(not checked)
        self.check_pyqt5.setChecked(not checked)
        self.check_psutil.setChecked(not checked)
        self.check_numpy.setChecked(not checked)
        self.check_pandas.setChecked(not checked)
        self.check_matplotlib.setChecked(not checked)
        self.check_scipy.setChecked(not checked)
        self.check_mdanalysis.setChecked(not checked)
        self.check_mdtraj.setChecked(not checked)
        self.check_seaborn.setChecked(not checked)
        self.check_joblib.setChecked(not checked)
        self.check_rdkit.setChecked(not checked)
        self.check_cuda.setChecked(not checked)
        self.check_gromacs.setChecked(not checked)
        self.check_openbabel.setChecked(not checked)
        self.check_gmxmmpbsa.setChecked(not checked)
        self.check_launcher.setChecked(not checked)
